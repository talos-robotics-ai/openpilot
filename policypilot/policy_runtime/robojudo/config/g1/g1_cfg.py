...
class g1_amo_real(RlPipelineCfg):
    """
    G1 real-robot AMO locomotion (balance / standing controller), 120 Hz low-level writer.

    Identical to ``g1_amo_arm_teleop_real`` but WITHOUT the ArmTeleopCtrlCfg
    override — the arms stay under the AMO policy. Used by policypilot's
    policy_manager as the default lower-body balance pipeline.
    """
...
@cfg_registry.register
class g1_amo_arm_teleop_real(RlPipelineCfg):
    """
    G1 real-robot AMO locomotion with merged 120 Hz low-level writer and arm teleop overrides.
    """
    ...
    ctrl: list[UnitreeCtrlCfg | ArmTeleopCtrlCfg] = [
        UnitreeCtrlCfg(),
        ArmTeleopCtrlCfg(
            frequency=120.0,
            solve_frequency=60.0,
            override_ttl=0.15,
            live_input_timeout=0.25,
            manual_enable=True,
            enable_key="r",
            disable_key="p",
            wrist_zmq=True,
            wrist_zmq_host="127.0.0.1",
            wrist_zmq_port=55556,
            render_head_image=False,
        ),
    ]
