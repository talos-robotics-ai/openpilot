...
class ArmTeleopCtrlCfg(CtrlCfg):
    ctrl_type: str = "ArmTeleopCtrl"

    frequency: float = 120.0
    solve_frequency: float = 60.0
    override_ttl: float = 0.2
    live_input_timeout: float = 0.25

    manual_enable: bool = False
    enable_key: str = "r"
    disable_key: str = "p"
    record_toggle_key: str = "s"

    input_mode: Literal["hand", "controller"] = "hand"
    display_mode: Literal["immersive", "ego", "pass-through"] = "immersive"

    wrist_zmq: bool = False
    wrist_zmq_host: str = "127.0.0.1"
    wrist_zmq_port: int = 55556

    record: bool = False
    record_frequency: float = 30.0
    record_max_queue_size: int = 2
    task_dir: str = "./teleop_data"
    task_name: str = "g1_inspire_live"
    task_goal: str = "task goal"
    task_desc: str = "task description"
    task_steps: str = "step1: do this; step2: do that;"
    ee: str | None = None
    hand_zmq_host: str = "127.0.0.1"
    hand_zmq_port: int = 55557
    record_rerun_log: bool = False

    render_head_image: bool = False
